...
array &activity = global_vars.arrays[4] ;
...
{ 
  ...
  array tmp = repro_rate * density + (interaction * density) * density;
  density +=tmp;

  /* generate a random permutation */
  array t(density.size);
  fillrand(t);
  sdensity += tmp[rank(t)];
...
NEWCMD(mutate,0)
{
  ...
  for (cell=0; cell<ncells; cell++)
      ...
      for (i=0; i<new_sp.size; i++)
	{
      ...
      new_sp = 1;
      l[density]  <<= new_sp;
      l[sdensity] <<= new_sp;  /* shadow model */
      new_sp = 0;
      l[create] <<=  new_sp;
      l[activity] <<=  new_sp;  /* shadow model */
  ...
  for (j=0; j<cell0->narr; j++)
    {
      array arr(ntrue);
      arr=-1.0;
      if (j==&activity-global_vars.arrays)  /* handle activity differently */
	{
	  for (i=0; i<ncells; i++)
	    {
	      arr=0;
	      arr[map[(*cell[i])[species]]] = (*cell[i])[activity];
	      (*cell[i])[activity]=arr;
	    }
	}
      else
...
NEWCMD(get,1)
{
  ...
  if (strcmp(argv[1],"sdensity")==0) result << g[sdensity];
  ...
  for (int i=0; i<g.ncells; i++)
    {
      ...
      result << (sum(v*v)/v.size - pow(sum(v)/v.size,2)) / 
	pow(sum(d)/d.size,2) << ' ';
    }
}
...
NEWCMD(newact,0)
{
  ...
  global g;
  get_all_globals(g);
  m=g[density]>50 && g[create]==0;
  r<<sum( m );
  cmd << "update_create";
}

NEWCMD(update_create,0)
{
  PARALLEL;
  create[gen_index( density>50 && create==0)]=tstep;
}
...
NEWCMD(act,0)
{
  ...
  get_all_globals(g);
#if 0
  if (activity.size<=max(species)) 
    { 
      array t(max(species)-activity.size+1);
      t=0;
      activity<<=t;
    }
#endif
  //  r<<sum(tstep-create,create>0);
  r<<sum(g[activity]);
}
...
NEWCMD(decimate,2)
{
  int num=atoi(argv[1]), den=atoi(argv[2]), i,j;
  for (i=j=0; i<density.size; i++, j++)
    {
      if (j==den) j=0;
      if (j<num) density[i]=0;
    }
}
